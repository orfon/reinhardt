...
const Lexer = exports.Lexer = function(templateString, origin) {
   Object.defineProperties(this, {
      templateString: {value: templateString},
      lineNo: {value: 1, writable: true},
      verbatim: {value: false, writable: true},
      origin: {value: origin}
   });
   return this;
};
...
Lexer.prototype.tokenize = function() {
   let inTag = false;
   const result = [];
   this.templateString.split(constants.tagRe).forEach(bit => {
      if (bit) {
...
Lexer.prototype.createToken = function(tokenString, inTag) {
   const blockContent = tokenString.slice(2, -2).trim();
   if (inTag && tokenString.startsWith(constants.BLOCK_TAG_START)) {
   ...
   if (inTag && !this.verbatim) {
      if (tokenString.startsWith(constants.VARIABLE_TAG_START)) {
         ...
         if (blockContent.startsWith('verbatim')) {
