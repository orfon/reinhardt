...
CycleNode.prototype.render = function(context) {
  if (! context.renderContext.has(this)) {
    context.renderContext.set(this, iter.cycle(this.cyclevars));
  }
  var cycleIter = context.renderContext.get(this);
  ...
  if (args.length > 4) {
    ...
    if (args.slice(-3)[0] == 'as') {
      if (args.slice(-1)[0] != 'silent') {
        throw new Error('Only "silent" flag is allowed after cycle\'s name and not "' + args.slice(-1) + '"');
      }
      asForm = true;
      silent = true;
      args = args.slice(0, -1);
...
IfChangedNode.prototype.render = function(context) {
  ...
  if (this.varList) {
    ...
    var compareTo = [this.nodeListTrue.render(context)];
  }
