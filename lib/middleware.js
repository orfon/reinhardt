...
const getErrorDetails = (error) => {
    ...
    const context = templateLines.slice(top, bottom).map(line => line.replace('\n', ''));
    return {
        templateName: templateSource[0].templateName,
        directories: templateSource[0].loader.templateDirs,
        lineNo: lineNo,
        before: before,
        during: during,
        after: after,
