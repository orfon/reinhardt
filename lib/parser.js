...
var Parser = exports.Parser = function(tokens) {
   this.tokens = tokens;
   this.tags = {};
   this.filters = {};
   this.addTags(require('./defaulttags'));
   this.addFilters(require('./defaultfilters'));
   return this;
}
Parser.prototype.parse = function(parseUntil) {
   ...
   while (this.tokens.length > 0) {
      ...
      if (token.tokenType === c.TOKEN_TEXT) {
         ...
         // FIXME debug only this.enterCommand(command, token);
         try {
...
Parser.prototype.extendNodeList = function(nodeList, node) {
   if (node.mustBeFirst && this.nodelist.length > 0) {
      throw new Errorr("must be first tag in template");
   }
...
Parser.prototype.emptyVariable = function(token) {
   throw new Error('Empty variable tag');
}

Parser.prototype.emptyBlockTag = function(token) {
   throw new Error('Empty block tag');
}

Parser.prototype.invalidBlockTag = function(token, command) {
   throw new Error('Invalid block tag "' + token + '" in ' + command);
}
Parser.prototype.unclosedBlockTag = function(parseUntil) {
   throw new Error('Unclosed tags: ' + parseUntil.join(', '));
}
