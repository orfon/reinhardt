...
exports.for = function(parser, token) {
  ...
  if (bits.length < 4) {
    throw new TemplateSyntaxError("'for' statements should have at least for words: " + token.contents);
  }
  ...
  if (args.length > 4) {
    ...
    if (args.slice(-3)[0] == 'as') {
      if (args.slice(-1)[0] != 'silent') {
        throw new TemplateSyntaxError('Only "silent" flag is allowed after cycle\'s name and not "' + args.slice(-1) + '"');
      }
      asForm = true;
      silent = true;
      args = args.slice(0, -1);
