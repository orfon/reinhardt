...
ExtendsNode.prototype.getParent = function(context) {
   ...
   if (!parent) {
      throw new Error('Invalid template name in extends tag: ' + parent);
   }
...
exports.block = function(parser, token) {
   var bits = token.splitContents();
   if (bits.length != 2) {
      throw new Error('block tag takes only one arguments');
   }
...
exports.extends = function(parser, token) {
   var bits = token.splitContents();
   if (bits.length != 2) {
      throw new Error('extends takes one argument');
   }
   var parentName = parser.compileFilter(bits[1]);
   var nodeList = parser.parse();
   if (nodeList.getByType(ExtendsNode).length > 0) {
      throw new Error('"extends" cannot appear more than once in the same template');
   }
