...
var Variable = exports.Variable = function(variable) {
    ...
    if (!isNaN(parseInt(variable))) {
        ...
        this.literal = variable.slice(1, -1);
    } else {
        if (variable == 'true') {
            this.literal = true;
...
Variable.prototype.resolve = function(context) {
    if (this.lookups) {
        ...
        for (var i = 0; i< this.lookups.length; i++) {
            ...
            // FIXME wtf is this 'items' stuff in dojo
            if (current.get && current.get instanceof Function) {
                current = current.get(part);
            } else if (current[part] === undefined) {
                current = c.TEMPLATE_STRING_IF_INVALID;
                break;
            } else {
