...
ForNode.prototype.render = function(context) {
    ...
    values.forEach((item, i) => {
        ...
        this.nodeListLoop.contents.forEach(node => {
...
exports.cycle = (parser, token) => {
    ...
    if (args[1].indexOf(',') > -1) {
        ...
        let pArgs = args[1].split(',').map(arg => '"' + arg + '"');
    ...
    if (args.length === 2) {
        // {% cycle foo %} case
        let name = args[1];
        if (!parser.namedCycleNodes.hasOwnProperty(name)) {
    ...
    if (asForm) {
        ...
        values = args.slice(1, -2).map(arg => parser.compileFilter(arg));
        node = new CycleNode(values, name, silent);
        parser.namedCycleNodes[name] = node;
    } else {
        values = args.slice(1).map(arg => parser.compileFilter(arg));
...
IfChangedNode.prototype.render = function(context) {
    if (context.has('forloop') && !context.get('forloop').hasOwnProperty(this.id)) {
    ...
    let compareTo;
    if (this.varList) {
        compareTo = this.varList.map(v => v.resolve(context, true));
        // FIXME catch except value error
    } else {
        compareTo = nodeListTrueOutput = [this.nodeListTrue.render(context)];
    }
    const isEqual = this.lastSeen !== undefined && compareTo.every((ct, idx) => {
...
exports.ifchanged = (parser, token) => {
    ...
    const values = bits.slice(1).map(bit => parser.compileFilter(bit));
