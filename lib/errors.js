   ...
   Object.defineProperty(this, 'body', {
   		get: function() {
   			...
   			var top = Math.max(0, lineNo - contextLines);
				...
				'<p><pre>',
                                before + '<span style="border-bottom: 1px solid red;">' + during + '</span>' +  after,
                                '</pre><p>',
                                 message || 'an error occured',
                                '</p></div><h2>',
                                this.templateSource[0].loadName,
                                ':</h2><pre>',
                        ].concat(context).concat(['</pre>', '<h2>Template search paths:</h2>']).concat(this.templateSource[0].dirs.join('\n'));
   		}
   })
   return this;
}
